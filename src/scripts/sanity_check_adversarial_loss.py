"""
Sanity check for adversarial loss using PGAN discriminator.
...
CONFIG = {
    ...
    "output_dir": "out/sanity_check_adv/",
...
def preprocess_for_discriminator(x):
    """Convert from [0,1] to [-1,1] for PGAN discriminator."""
    return x * 2.0 - 1.0


def evaluate_discriminator_on_images(discriminator, images, name=""):
    """Evaluate discriminator scores on a batch of images."""
    with torch.no_grad():
        # Preprocess: [0,1] -> [-1,1]
        images_prep = preprocess_for_discriminator(images)
...
def main():
    print("=" * 60)
    print("Adversarial Loss Sanity Check")
    print("=" * 60)
    
    os.makedirs(CONFIG["output_dir"], exist_ok=True)
    
    # Load discriminator
    print("\n[1] Loading PGAN discriminator...")
    ...
    for checkpoint_path, name in BETA_VAE_CHECKPOINTS:
        ...
        for batch_idx, (images, _) in enumerate(dataloader):
            ...
            result = evaluate_discriminator_on_images(discriminator, recon, f"{name}_batch_{batch_idx}")
