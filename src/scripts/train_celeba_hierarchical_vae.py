...
CONFIG = {
    ...
    "sobel_weight": 0.25,
    "sobel_loss_type": "L2",
    "adv_weight": 0.001,  # adversarial loss weight (0 to disable)
    "celeb_path": "./data/",
    "device": "cuda" if torch.cuda.is_available() else "cpu",
    "project_name": "gm-variational",
    "run_name": "celeba-hvae-kl0.000001-sobel0.25-adv0.001-ep40",
    "seed": 42,
    "num_workers": 8,
}
...
def loss_function(recon_x, x, kl_losses, config, discriminator=None):
...
def train(model, train_loader, optimizer, epoch, config, discriminator=None):
    ...
    for batch_idx, (data, _) in pbar:
        ...
        loss, mse, kld, sobel, adv = loss_function(recon_batch, data, kl_losses, config, discriminator)
        loss.backward()
